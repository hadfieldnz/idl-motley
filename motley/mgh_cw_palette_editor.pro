...
; **************************************************************************
;
; This file is a copy of cw_palette_editor.pro as distributed with IDL
; 5.6 (see original version information below) with the following
; changes:
;
; - All routine and structure names have been modified by applying the
;   string replacement, "cw_pal" -> "mgh_cw_pal".
;
; - A FILE keyword has been added to the following routines to allow
;   the color table file to be specified:
;     MGH_CW_PALETTE_EDITOR
;     MGH_CW_PALETTE_EDITOR_GET
;     MGH_CW_PALETTE_EDITOR_SET
;    See the documentation headers for details.
;
; - The widget for selecting predefined color tables has been changed
;   from a pull-down menu to a droplist. This change was made to
;   support the FILE keyword to MGH_CW_PALETTE_EDITOR_SET, as a
;   WIDGET_DROPLIST, unlike a CW_PDMENU, can have its list of names
;   changed after initialisation. In any case, I think a droplist
;   widget is a more appropriate widget where a user needs to make a
;   selection.
;
; - The setting of the NOTIFY_REALIZE keyword during widget
;   initialisation has been delayed. This change allows a palette
;   editor widget to function proeprly when added to an
;   already-realised base.
;
; Mark Hadfield, 2002-12-23
;
; **************************************************************************

; $Id$
;
; Copyright (c) 1993-2002, Research Systems, Inc.  All rights reserved.
...
;
; CALLING SEQUENCE:
;   MGH_CW_PALETTE_EDITOR_GET, widgetID
;
; INPUTS:
...
;
; CALLING SEQUENCE:
;   MGH_CW_PALETTE_EDITOR_SET, widgetID
;
; INPUTS:
;   widgetID:    The widget ID of the MGH_CW_PALETTE_EDITOR compound widget.
;
...
PRO mgh_cw_paledit_notImplemented, parent
...
function mgh_cw_paledit_getviewportsize, state
...
FUNCTION mgh_cw_paledit_getDataCoordsfromViewport, state, viewportX, viewportY
...
FUNCTION mgh_cw_paledit_getXDataOfViewportMinMax, state

    COMPILE_OPT HIDDEN, STRICTARR


    ; get current viewport size
    viewportsize = mgh_cw_paledit_getviewportsize(state)

    dataCoordsLeft = mgh_cw_paledit_getDataCoordsFromViewport(state, 0, 0)
    dataCoordsRight = mgh_cw_paledit_getDataCoordsFromViewport(state, $
       viewportsize[0]-1, 0)
    return, [dataCoordsLeft[0], dataCoordsRight[0]]

end

FUNCTION mgh_cw_paledit_getXVirtSize, draw
...
PRO mgh_cw_paledit_setYVirtSize, state, newy
...
PRO mgh_cw_paledit_setXVirtSize, state, newx
    ...
    if (newx gt state.xsize) then begin
       mgh_cw_paledit_setYVirtSize, state, state.ysize-scrollbarHeight
    endif else begin
       mgh_cw_paledit_setYVirtSize, state, state.ysize
    endelse

end

FUNCTION mgh_cw_paledit_getXViewportLoc, view, draw
...
PRO mgh_cw_paledit_setXViewportLoc, view, draw, newvirtx
...
FUNCTION mgh_cw_paledit_curvesEditable, state
...
FUNCTION mgh_cw_paledit_curvesDisplayed, state
...
FUNCTION mgh_cw_paledit_scaleToDataSpace, state, yData
...
FUNCTION mgh_cw_paledit_scaleFromDataSpace, state, yData
...
FUNCTION mgh_cw_paledit_getPolylineData, state, index
    ...
    data = mgh_cw_paledit_scaleFromDataSpace(state, index, data)
    return, data[1,*]
end


PRO mgh_cw_paledit_setPolylineData, state, yData

    COMPILE_OPT HIDDEN, STRICTARR

    ; In most cases this should not be called directly,
    ; since it is called by mgh_cw_paledit_setAllData
    ;
...
PRO mgh_cw_paledit_setPaletteData, state, oPalette, yData
    ...
    yData = mgh_cw_paledit_scaleFromDataSpace(state, yData)
...
FUNCTION mgh_cw_paledit_getPaletteData, oPalette, index
...
FUNCTION mgh_cw_paledit_getColorVectorData, state
    ...
    ; curves 0,1,2 are r,g,b (or curves for other color system)
    ; curve 3 is alpha (if applicable)
    yTemp = mgh_cw_paledit_scaleToDataSpace(state, state.colorVectors)
    return, yTemp
end


PRO mgh_cw_paledit_setColorVectorData, state, yData
...
PRO mgh_cw_paledit_setAllData, state, yData

    COMPILE_OPT HIDDEN, STRICTARR


    ; scale from the data space to the color space
    yTemp = mgh_cw_paledit_scaleFromDataSpace(state, yData)
    mgh_cw_paledit_setColorVectorData, state, yTemp

    ; always use the data space (not color space) for polylines
    mgh_cw_paledit_setPolylineData, state, yData

    ; use the unscaled data since setPaletteData does the scaling
    mgh_cw_paledit_setPaletteData, state, state.oPaletteMain, yData

;   widget_control, state.wDroplistPredefined, SET_DROPLIST_SELECT=-1
    widget_control, state.wDroplistPredefined, SET_LIST_SELECT=-1

end


FUNCTION mgh_cw_paledit_buildEventPaletteModified, state, wAppTop, handler
...
FUNCTION mgh_cw_paledit_buildEventSelectionMoved, state, wAppTop, handler
...
PRO mgh_cw_paledit_modifyByIndex, state, xval, yval
    ...
    modifySettings = mgh_cw_paledit_curvesEditable(state)
    data=mgh_cw_paledit_getColorVectorData(state)
    for i=0,3 do begin
       if (modifySettings[i] gt 0) then begin
          data[i, updateX : (updateX + indexIncr)]=updateY
       endif
    endfor
    mgh_cw_paledit_setAllData, state, data

end

PRO mgh_cw_paledit_modifyBySegment, state, xval, yval
    ...
    mgh_cw_paledit_setAllData, state, state.colorVectorsTemp
    mgh_cw_paledit_setAllData, state, data

end


PRO mgh_cw_paledit_modifyBySlide, state, xval, yval
...
PRO mgh_cw_paledit_modifyByBarrel, state, xval, yval
...
PRO mgh_cw_paledit_modifyByStretch, state, xval, yval
...
PRO mgh_cw_paledit_modifyByCurveType, state, type
...
PRO mgh_cw_paledit_EditPredefined, state, index

    COMPILE_OPT HIDDEN, STRICTARR

    oPaletteTemp = OBJ_NEW('IDLgrPalette')
    ; skip the first menu item
    oPaletteTemp->LoadCT, index, FILE=state.file
    ...
    1: begin ; RGB to HSV
       ...
       yTemp[1,*] = cVecOut1
       yTemp[2,*] = cVecOut2
       dataTemp = mgh_cw_paledit_scaleToDataSpace(state, yTemp)
    end
    2: begin ; RGB to HLS
       COLOR_CONVERT, aRed, aGreen, aBlue, $
                      cVecOut0, cVecOut1, cVecOut2, $
...
PRO mgh_cw_paledit_SetFile, state

    COMPILE_OPT HIDDEN, STRICTARR

    file = dialog_pickfile(TITLE='Select color table file', /MUST_EXIST, FILTER='*.tbl')
    if file ne '' then mgh_cw_palette_editor_set, state.wCWPalEdTop, FILE=file
...
PRO mgh_cw_paledit_zoom_toselection, state

    COMPILE_OPT HIDDEN, STRICTARR


    ; get virtual size
    xsize_virt = mgh_cw_paledit_getXVirtSize(state.wDraw)
    ...
    xDataOfViewportMinMax = mgh_cw_paledit_getXDataOfViewportMinMax(state)
    ...
    mgh_cw_paledit_setXVirtSize, state, xsize_virt_new
    ...
    mgh_cw_paledit_setXViewportLoc, state.oView, state.wDraw, x1sel_virt_new

    ; scale changed, scale the width of the drag handles
    mgh_cw_paledit_DragSetWidth, state, xsize_virt_new

end


PRO mgh_cw_paledit_zoom_byfactor, state, scale_factor
   ...
   if xsize_virt * scale_factor LE 256 then begin
      mgh_cw_paledit_zoom_1to1, state
      return
   endif

   xviewportloc_virt = mgh_cw_paledit_getXViewportLoc(state.oView, state.wDraw)
   ...
   mgh_cw_paledit_setXVirtSize, state, xsize_virt_new
   mgh_cw_paledit_setXViewportLoc, state.oView, state.wDraw, xviewportloc_virt_new
   ; scale changed, scale the width of the drag handles
   mgh_cw_paledit_DragSetWidth, state, xsize_virt_new
...
PRO mgh_cw_paledit_zoom_1to1, state
    ...
    mgh_cw_paledit_setXVirtSize, state, viewportsize[0]
    xRescaled = 0
    mgh_cw_paledit_setXViewportLoc, state.oView, state.wDraw, xRescaled

    ; scale changed, scale the width of the drag handles
    mgh_cw_paledit_DragSetWidth, state, viewportsize[0]

end

; Routines to update display of data values

PRO mgh_cw_paledit_DisplayDataVals, state, xData, yData
    ...
    colorVectors = mgh_cw_paledit_getColorVectorData(state)
    yVals = colorVectors[*, fix(xData)]
    ; scale from the data space to the color space
    yScaled = mgh_cw_paledit_scaleFromDataSpace(state, yVals)
    for i=0,2 do begin
...
PRO mgh_cw_paledit_DragSetWidth, state, xsize_virt_new
...
PRO mgh_cw_paledit_DragSetLoc, wDraw, oPolygon, newX
   ...
   ; scale the width of the drag handles down if zoomed in
   xsize_virt = mgh_cw_paledit_getXVirtSize(wDraw)
...
FUNCTION mgh_cw_paledit_getvalue, wId
    ...
    colors[0,*] = mgh_cw_paledit_getPaletteData(state.oPaletteMain, 0)
    colors[1,*] = mgh_cw_paledit_getPaletteData(state.oPaletteMain, 1)
    colors[2,*] = mgh_cw_paledit_getPaletteData(state.oPaletteMain, 2)
...
FUNCTION mgh_cw_paledit_CheckData, inData, $
                               outPalette, $
                               outPaletteSupplied, $
                               outValues, $
                               outAlphaInUse
...
PRO mgh_cw_paledit_setvalue, wId, value

    COMPILE_OPT HIDDEN, STRICTARR


    if (mgh_cw_paledit_CheckData(value, $
                             outPalette, $
                             outPaletteSupplied, $
                             outValues, $
                             outAlphaInUse) le 0) then begin
       message, 'mgh_cw_palette_editor: set_value: Invalid argument'
       return
    endif
    ...
    mgh_cw_paledit_setAllData, state, outValues[0:2+state.alphaInUse,*]
...
FUNCTION mgh_cw_paledit_DrawHandler, ev
    ...
    if ((ev.type EQ 0) or (ev.type EQ 2)) then begin
       ...
       if (ev.type eq 2) then begin
          viewport_size = mgh_cw_paledit_getviewportsize(state)
          ev.x = ev.x > 0
       ...
       dataCoords = mgh_cw_paledit_getDataCoordsFromViewport(state, ev.x, ev.y)
       ...
       mgh_cw_paledit_DisplayDataVals, state, $
       ...
       if (state.buttonDownDragArea eq 1) then begin
          ...
          if (state.handle_in_use eq 0) then begin
             ...
             mgh_cw_paledit_DragSetLoc, state.wDraw, state.oPolygonDragLeft, $
                new_left
             ...
             mgh_cw_paledit_DragSetLoc, state.wDraw, state.oPolygonDragRight, $
                new_right
          ...
          SelectionSetLoc, state.oPolygonSelection, $
             new_left, new_right
          state.oWindow->Draw, state.oView
          event = mgh_cw_paledit_buildEventSelectionMoved(state, $
             ev.top, ev.handler)
          widget_control,wCWPalEdMainBase,set_uvalue=state
          return, event
       endif
       ...
       if (state.buttonDownEditArea eq 1) then begin
          ...
          if (TOTAL(mgh_cw_paledit_curvesEditable(state)) gt 0) then begin
             if (state.editMode eq 0) then begin
                ; freehand
                mgh_cw_paledit_modifyByIndex, state, $
                   dataCoords[0], dataCoords[1]
                state.oWindow->Draw, state.oView
                event = mgh_cw_paledit_buildEventPaletteModified(state, $
                   ev.top, ev.handler)
             endif
             if (state.editMode eq 1) then begin
                ; line segment
                mgh_cw_paledit_modifyBySegment, state, $
                   dataCoords[0], dataCoords[1]
                state.oWindow->Draw, state.oView
                event = mgh_cw_paledit_buildEventPaletteModified(state, $
                   ev.top, ev.handler)
             ...
             if (state.editMode eq 2) then begin
                ; stretch
                mgh_cw_paledit_modifyByBarrel, state, $
                   dataCoords[0], dataCoords[1]
                state.oWindow->Draw, state.oView
                event = mgh_cw_paledit_buildEventPaletteModified(state, $
                   ev.top, ev.handler)
             ...
             if (state.editMode eq 3) then begin
                ; slide
                mgh_cw_paledit_modifyBySlide, state, $
                   dataCoords[0], dataCoords[1]
                state.oWindow->Draw, state.oView
                event = mgh_cw_paledit_buildEventPaletteModified(state, $
                   ev.top, ev.handler)
             ...
             if (state.editMode eq 4) then begin
                ; stretch
                mgh_cw_paledit_modifyByStretch, state, $
                   dataCoords[0], dataCoords[1]
                state.oWindow->Draw, state.oView
                event = mgh_cw_paledit_buildEventPaletteModified(state, $
                   ev.top, ev.handler)
...
PRO mgh_cw_paledit_displayModifyButtonHandler, ev
...
PRO mgh_cw_paledit_zoomButtonHandler, ev
    ...
    case uvalue of
       'ZOOM_SEL': begin
          mgh_cw_paledit_zoom_toselection, state
          state.oWindow->Draw, state.oView
          widget_control,wCWPalEdMainBase,set_uvalue=state
       end
       'ZOOM_IN': begin
          scale_factor = 2.
          mgh_cw_paledit_zoom_byfactor, state, scale_factor
          state.oWindow->Draw, state.oView
          widget_control,wCWPalEdMainBase,set_uvalue=state
       end
       'ZOOM_OUT': begin
...
PRO mgh_cw_paledit_colorSpaceHandler, ev
    ...
    cVecs = mgh_cw_paledit_getColorVectorData(state)

    cVecs = mgh_cw_paledit_scaleFromDataSpace(state, cVecs)
    case ev.index of
       0: begin ; RGB
    ...
    cVectorsTemp = mgh_cw_paledit_scaleToDataSpace(state, colorVectors)
    mgh_cw_paledit_setPolylineData, state, cVectorsTemp
    mgh_cw_paledit_setColorVectorData, state, colorVectors
...
PRO mgh_cw_paledit_editModeHandler, ev
    ...
    case ev.index of
       ...
       3:
       4: begin
             ;;;mgh_cw_paledit_notImplemented, state.wCWPalEdTop
             ;;;ev.index = 0
...
FUNCTION mgh_cw_paledit_OperateButtonHandler, ev
    ...
    case ev.id of
       state.wButtonRamp: mgh_cw_paledit_modifyByCurveType, state, 'RAMP'
       state.wButtonSmooth: mgh_cw_paledit_modifyByCurveType, state, 'SMOOTH'
       state.wDroplistPredefined: mgh_cw_paledit_editPredefined, state, ev.index
       state.wButtonReverse: mgh_cw_paledit_modifyByCurveType, state, 'REVERSE'
       state.wButtonInvert: mgh_cw_paledit_modifyByCurveType, state, 'INVERT'
       state.wButtonPosterize: mgh_cw_paledit_modifyByCurveType, state, 'POSTERIZE'
       state.wButtonDuplicate: mgh_cw_paledit_modifyByCurveType, state, 'DUPLICATE'
       state.wButtonFile: begin
         mgh_cw_paledit_setFile, state
         return, 0L
       end
    else:
    endcase
...
PRO mgh_cw_paledit_nullEventHandler, ev
...
FUNCTION mgh_cw_paledit_event, ev
...
PRO mgh_cw_paledit_cleanup, wCWPalEdMainBase
...
PRO mgh_cw_paledit_init, id
   ...
   mgh_cw_paledit_DisplayDataVals, state, 0, 0

   SelectionSetLoc, state.oPolygonSelection, state.sel_x1, state.sel_x2
   mgh_cw_paledit_DragSetLoc, state.wDraw, state.oPolygonDragLeft, state.sel_x1
   mgh_cw_paledit_DragSetLoc, state.wDraw, state.oPolygonDragRight, state.sel_x2
   mgh_cw_paledit_DragSetWidth, state, mgh_cw_paledit_getXVirtSize(state.wDraw)
...
PRO MGH_CW_PALETTE_EDITOR_SET, id, $
    ...
    if (N_ELEMENTS(alpha) ne 0) then begin
       ...
       1: begin   ; Alpha set to vector, add to display
          ...
          if (dims[0] ne 256) then $
             message, 'mgh_cw_palette_editor: ALPHA: Invalid argument'
       ...
       else: message, 'mgh_cw_palette_editor: ALPHA: Invalid argument'
       endcase
    endif

    if (N_ELEMENTS(file) ne 0) then begin
       state.file = file
       LOADCT, GET_NAMES=paletteNames, FILE=state.file
       WIDGET_CONTROL, state.wDroplistPredefined, SET_VALUE=paletteNames
    endif
    ...
    if (N_ELEMENTS(histogram) ne 0) then begin
       ...
       1: begin   ; Histogram set to vector, add to display
          ...
          if (dims[0] ne 256) then $
             message, 'mgh_cw_palette_editor: HISTOGRAM: Invalid argument'
       ...
       else: message, 'mgh_cw_palette_editor: HISTOGRAM: Invalid argument'
       endcase
    endif
...
PRO MGH_CW_PALETTE_EDITOR_GET, id, $
    ALPHA=alpha, $                    ; OUT: Alpha vector
    FILE=file, $                      ; OUT: Color table file name
...
FUNCTION MGH_CW_PALETTE_EDITOR, parent, $
        DATA=data, $
        FILE = file, $
        FRAME = frame, $
        HISTOGRAM=histogram, $
    ...
    if (not keyword_set(uname)) then uname = 'MGH_CW_PALETTE_EDITOR_UNAME'
    ...
    if (n_elements(selection) eq 0) then begin
       ...
       if ((ndims ne 1) or (dims[0] ne 2)) then $
          message, 'mgh_cw_palette_editor: SELECTION: Invalid argument'
    ...
    validData = mgh_cw_paledit_CheckData(data, $
                                     outPalette, $
                                     outPaletteSupplied, $
                                     outValues, $
                                     outAlphaInUse)
    ...
    wCWPalEdTop=widget_base(parent, $
        ...
        FRAME=frame, $
        UNAME=uname, $
        FUNC_GET_VALUE = 'MGH_CW_PALEDIT_GETVALUE', $
        PRO_SET_VALUE  = 'MGH_CW_PALEDIT_SETVALUE')
    ...
    if horizontal then begin
       wCWPalEdMainBase = widget_base(wCWPalEdTop, $
          KILL_NOTIFY = 'MGH_CW_PALEDIT_CLEANUP', $
    ...
    wDraw=widget_draw(drawbase, graphics=2, $
       /button_events, $
       EVENT_FUNC='mgh_cw_paledit_DrawHandler', $
       /motion_events, $
       /app_scroll, $
       retain=0, $
    ...
    zoomrow = widget_base(cntrlbase, $
       EVENT_PRO = 'MGH_CW_PALEDIT_ZOOMBUTTONHANDLER', $
       UVALUE=wCWPalEdMainBase, $
    ...
    wColorSpace = Widget_Droplist(cntrlbase, $
       EVENT_PRO='mgh_cw_paledit_ColorSpaceHandler', $
       UVALUE=wCWPalEdMainBase, $
       VALUE=value, $
       TITLE='Color Space' ,$
    ...
    wEditMode = Widget_Droplist(cntrlbase, $
       TITLE='Edit Mode', $
       EVENT_PRO='mgh_cw_paledit_EditModeHandler', $
       UVALUE=wCWPalEdMainBase, $
    ...
    operaterow = widget_base(cntrlbase, $
       EVENT_FUNC = 'MGH_CW_PALEDIT_OPERATEBUTTONHANDLER', $
       UVALUE=wCWPalEdMainBase, $
       COLUMN=3, $
   ...
   operaterow3 = widget_base(cntrlbase, $
    ...
    LOADCT, GET_NAMES=paletteNames, FILE=file
    wDroplistPredefined = WIDGET_LIST(operaterow3, YSIzE=5, $
       VALUE=paletteNames, $
       UVALUE=wCWPalEdMainBase, $
       UNAME=uname+'_DROPLISTPREDEFINED')

    wButtonFile = Widget_Button(operaterow3, $
       UVALUE=wCWPalEdMainBase, $
       VALUE='File', $
       UNAME=uname+'_FILE')
    ...
    wDisplayModifyBase = widget_base(cntrlbase, $
       EVENT_PRO='mgh_cw_paledit_DisplayModifyButtonHandler', $
       COLUMN=4, $
       SPACE=1, $
       XPAD=1, $
       YPAD=1, $
    ...
    ; Create some lights.
    oLightDirectional = OBJ_NEW('IDLgrLight', LOCATION=[0,0,5], $
       TYPE=2, INTENSITY=0.75)
    omTop->Add, oLightDirectional
    oLightAmbient = OBJ_NEW('IDLgrLight', TYPE=0, INTENSITY=0.25)
    omTop->Add,oLightAmbient
    ...
    oPolygonSelection = OBJ_NEW('IDLgrPolygon', $
       bottom=[255,255,255], $
       color=[255,255,255])
    ...
    state = {  $
       ...
       wDroplistPredefined: wDroplistPredefined, $
       wButtonFile: wButtonFile, $
    ...
    widget_control, wCWPalEdtop,  NOTIFY_REALIZE = 'MGH_CW_PALEDIT_INIT'
    if widget_info(parent, /VALID_ID) then if widget_info(parent, /REALIZED) then $
        mgh_cw_paledit_init, wCWPalEdtop
