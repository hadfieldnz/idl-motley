...
;      Forced some denominators to (double precision) floating point.
...
function mgh_polyfillg, poly, $
  ...
  if n_elements(xout)*n_elements(yout) eq 0 then begin
    ...
    if n_elements(delta) eq 0 then $
      delta = [xmax-xmin,ymax-ymin]/double(dimension-1)
  ...
  if xy2d then begin
    ...
    for j=j0,j1 do begin
      pola = poly
      pola = mgh_polyclip(pola, yout[j], 1, 0)
      pola = mgh_polyclip(pola, yout[j+1], 1, 1)
      for i=i0,i1 do begin
        polc = pola
        polc = mgh_polyclip(polc, xout[i], 0, 0)
        polc = mgh_polyclip(polc, xout[i+1], 0, 1)
        if size(polc, /N_DIMENSIONS) eq 2 then begin
          ...
          if ~ keyword_set(area) then begin
            ...
            result[i,j] = result[i,j] / double(aa)
          endif
        endif
      endfor
    endfor
